	...
	void rotate(string type, vector<vector<int>>& grid)
	{
		...
		if (canRotate(pivot_x, pivot_y))
		{
			...
			if (state == 1)
			{
				try
				{
					if (grid.at(grid_y).at(grid_x - 1) == 0 && grid.at(grid_y - 1).at(grid_x) == 0 && grid.at(grid_y - 1).at(grid_x + 1) == 0)
					{
			...
			if (state == 2)
			{
				try
				{
					if (grid.at(grid_y + 1).at(grid_x) == 0 && grid.at(grid_y).at(grid_x + 1) == 0 && grid.at(grid_y + 1).at(grid_x + 1) == 0)
					{
